...
# A01476037 & A01972388
#
# I first changed this program by allowing it to complete the command for IIADDQ following the procedure used in part B of this lab assignment.
#
# Next I changed the code to allow load forwarding from the IMRMOVQ instruction to the IRMMOVQ instruction. I did this by changing the load/use detection hazard to not inject bubbles into the pipe when there is a IMRMOVQ instruction followed by a IRMMOVQ instruction that uses the same register. Next I changed the e_valA definition in the execute stage to make it equal to m_valM in the case that there is a IMRMOVQ next to a IRMMOVQ that uses the same register. This way the loaded value is forwarded to the store value of the next instruction.
#
# Finally I changed the branch predicting logic. The original logic always assumed the conditional branches were taken. This has about a 60% accuracy. I changed this to a Backwards Taken Forwards Not Taken (BTFNT) branch prediction. If the branch taken value is less than the branch not taken value, then the taken branch is predicted. And if the branch taken value is greater than or equal to the branch not taken value, the the not taken branch is predicted. This has a slightly higher accuracy of about 65%. 
...
word e_valA = [
	E_icode == IRMMOVQ && M_dstM == E_srcA : m_valM; 
...
bool F_stall =
	...
	E_icode in { IMRMOVQ } && D_icode in { IOPQ, IRET, IPOPQ, IRRMOVQ  }
	 && E_dstM in { d_srcA, d_srcB } ||
